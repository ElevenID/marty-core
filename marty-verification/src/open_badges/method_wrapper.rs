    ...
    pub fn from_json(value: &Value) -> VerificationResult<Self> {
        ...
        if let Some(type_str) = value.get("type").and_then(|v| v.as_str()) {
            if type_str == "X509VerificationKey2021" {
                let x509: X509VerificationKey2021 = serde_json::from_value(value.clone())
                    .map_err(|e| VerificationError::open_badges(
                        format!("Failed to parse X509VerificationKey2021: {}", e)
                    ))?;
        ...
        let method = if let Ok(generic) = 
            ...
            AnyMethod::try_from(generic)
                .map_err(|e| VerificationError::open_badges(format!("Invalid verification method: {}", e)))?
        } else {
            serde_json::from_value::<AnyMethod>(value.clone())
                .map_err(|e| VerificationError::open_badges(format!("Failed to parse verification method: {}", e)))?
        };
...
pub fn parse_open_badge_method(
    ...
    match OpenBadgeMethod::from_json(value) {
        Ok(method) => {
            match IriBuf::new(method.id().to_string()) {
