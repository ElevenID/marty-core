...
use crate::trust_anchor::eudi::{EudiRegistry, EuMemberState, TrustServiceProvider, TrustService, TspStatus};

#[cfg(feature = "eudi-client")]
use crate::pkd::eudi_xml::{TrustServiceStatusList, LocalizedName};
...
impl EudiLotlClient {
    ...
    pub async fn fetch_lotl(&self) -> VerificationResult<Vec<TrustedListInfo>> {
        let response = self.http_client
            ...
            .send()
            .await
            .map_err(|e| VerificationError::pkd_fetch(
                &self.lotl_url,
                format!("Failed to fetch LoTL: {}", e)
            ))?;
    ...
    fn parse_lotl_xml(&self, xml: &str) -> VerificationResult<Vec<TrustedListInfo>> {
        ...
        let tsl: TrustServiceStatusList = from_str(xml)
            .map_err(|e| *VerificationError::der_error(format!(
                "Failed to parse LoTL XML: {}", e
            )))?;
        ...
        if let Some(pointers) = tsl.pointers {
            for pointer in pointers.pointers {
                ...
                let territory = pointer.additional_info
                    .and_then(|ai| {
                        ai.other_info.iter()
                            .find_map(|oi| oi.territory.clone())
                    })
                    .unwrap_or_default();
                ...
                let issue_date = chrono::DateTime::parse_from_rfc3339(&tsl.scheme_information.issue_date)
                    .ok()
                    .map(|dt| dt.with_timezone(&Utc))
                    .unwrap_or_else(Utc::now);
                
                let next_update = tsl.scheme_information.next_update
                    .as_ref()
    ...
    pub async fn fetch_member_state_tl(
        ...
        let tl_info = tl_infos
            ...
            .ok_or_else(|| *VerificationError::io_error(
                format!("No TL found for member state {}", member_state.code())
            ))?;
        ...
        let xml = response.text().await
            .map_err(|e| VerificationError::pkd_fetch(
                &tl_info.url,
                format!("Failed to read TL response: {}", e)
            ))?;
    ...
    fn parse_tl_xml(
        ...
        if let Some(tsp_list) = tsl.tsp_list {
            for xml_tsp in tsp_list.providers {
                ...
                for xml_svc in xml_tsp.services.services {
                    ...
                    let certificates: Vec<String> = xml_svc.info.digital_identity
                        .digital_id
                        .iter()
                ...
                let tsp_status = if trust_services.iter().any(|s| 
                    s.status.contains("granted") || s.status.contains("Granted")
                ) {
                    TspStatus::Granted
                } else if trust_services.iter().any(|s| 
                    s.status.contains("withdrawn") || s.status.contains("Withdrawn")
                ) {
                    TspStatus::Withdrawn
                } else if trust_services.iter().any(|s| 
                    s.status.contains("suspended") || s.status.contains("Suspended")
                ) {
                    TspStatus::Suspended
                } else {
                    TspStatus::Unknown
        ...
        tracing::info!(
            ...
            member_state.code(),
            providers.len(),
            providers.iter().map(|p| p.trust_services.len()).sum::<usize>()
        );
    ...
    #[cfg(feature = "eudi-client")]
    pub async fn sync_registry(&self, registry: &mut EudiRegistry) -> VerificationResult<LotlSyncResult> {
        use base64::Engine;
        use der::Decode;
        ...
        for tl_info in &tl_infos {
            ...
            match self.fetch_member_state_tl(member_state).await {
                        ...
                        for service in &provider.trust_services {
                            for cert_b64 in &service.certificates {
                                ...
                                let cert_der = base64::engine::general_purpose::STANDARD
                                    .decode(cert_b64)
                                    .map_err(|e| *VerificationError::der_error(
                                        format!("Invalid base64 cert: {}", e)
                                    ))?;
                                
                                match Certificate::from_der(&cert_der) {
                                    Ok(cert) => {
                                        // Determine trust purpose from service type
                                        let trust_purpose = if service.service_type.contains("QCertESig") 
                ...
                Err(e) => {
                    tracing::warn!(
                        "Failed to fetch TL for {}: {}",
                        tl_info.scheme_territory, e
                    );
                }
            }
        }
        ...
        tracing::info!(
            ...
            anchors_added, qtsps_discovered, member_states.len()
        );
...
impl EudiLotlClient {
    ...
    pub async fn sync_registry(&self, _registry: &mut EudiRegistry) -> VerificationResult<LotlSyncResult> {
        Err(VerificationError::NotSupported(
            "EUDI client feature not enabled".to_string()
        ))
    }
...
<TrustServiceStatusList xmlns="http://uri.etsi.org/02231/v2#">
    ...
    fn test_member_state_filter() {
        let client = EudiLotlClient::new()
            .with_member_states(vec![EuMemberState::DE, EuMemberState::FR]);
    ...
    fn test_parse_lotl_with_filter() {
        let client = EudiLotlClient::new()
            .with_member_states(vec![EuMemberState::DE]);
        let result = client.parse_lotl_xml(SAMPLE_LOTL_XML);
        assert!(result.is_ok());
    ...
    fn test_parse_tl_xml() {
        ...
        assert_eq!(providers[0].trust_services[0].name, "Qualified Certificate Service");
    }
    ...
    fn test_parse_service_status() {
        assert_eq!(
            EudiLotlClient::parse_service_status("http://uri.etsi.org/TrstSvc/TrustedList/Svcstatus/granted"),
            TspStatus::Granted
        );
        assert_eq!(
            EudiLotlClient::parse_service_status("http://uri.etsi.org/TrstSvc/TrustedList/Svcstatus/withdrawn"),
            TspStatus::Withdrawn
        );
        assert_eq!(
            EudiLotlClient::parse_service_status("http://uri.etsi.org/TrstSvc/TrustedList/Svcstatus/suspended"),
            TspStatus::Suspended
        );
        assert_eq!(
